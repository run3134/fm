...
def fm_gradient_sgd_trick(X, y, V0, V, W, regParam):
    """
    Computes the gradient for one instance using Rendle FM paper (2010) trick (linear time computation)
    X is an array of (p,),
    """
    xa = np.array([X]) # matrix of 1xp
    x_matrix = xa.T.dot(xa) # pxp
    x_sqaure = xa*xa
    ...
    WX =  xa.dot(W)
    WX_square = (xa*xa).dot(W*W)
    DW1 = np.dot(x_matrix, W)
    DW2 = np.diag(x_sqaure.flatten()).dot(W)
...
class FactMachine():
    def __init__(self, factorLength=4, alpha=0.01, regParam=0.01, iterations=2, toScale=True):
        self.m = None
        self.feat = None

        self.toScale = toScale
        self.factorLength = factorLength
        self.alpha = alpha
        self.regParam = regParam
        self.iterations = iterations

    def fit(self, X, Y, feat_list):
        '''
        need to store feature_list used in model which is consistent with the coef
        in prediction
        :param data:
            data is in spark
            has x and y
        ...
        '''
        self.feat = feat_list
        ## organize the data's columns
        return self

    def predict(self, X):
        '''
        if feat_list is not None: check feature list
        else check dim between model and X

        :param data:
        :return: yhat on test data
        '''
